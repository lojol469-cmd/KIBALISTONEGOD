...
# type: ignore
"""
Application SETRAGESTION - Système de gestion avancé avec bases de données par utilisateur
...
# Configuration Email (utilise les variables existantes de .env)
EMAIL_SENDER = os.environ.get('EMAIL_USER', 'notification@setragestion.com')
EMAIL_PASSWORD = os.environ.get('EMAIL_PASS', '')
EMAIL_SMTP_SERVER = os.environ.get('EMAIL_SMTP_SERVER', 'smtp.gmail.com')
EMAIL_SMTP_PORT = int(os.environ.get('EMAIL_SMTP_PORT', '587'))

# ===== SYSTÈME DE NOTIFICATION EMAIL (SMTP - DÉSACTIVÉ) =====
# Note: Le système via backend Node.js est utilisé à la place
def send_email_notification_smtp(to_email, subject, body, action_type="info"):
    ...
    Args:
        ...
        action_type: Type d'action (info, habilitation, enregistrement, alerte)
    """
    try:
        # Ne pas envoyer si pas configuré
        if not EMAIL_PASSWORD or EMAIL_PASSWORD == '':
            print(f"⚠️ Email non envoyé (pas configuré): {subject} à {to_email}")
            return False
        
        msg = MIMEMultipart('alternative')
        msg['From'] = EMAIL_SENDER
        msg['To'] = to_email
        msg['Subject'] = f"[SETRAGESTION] {subject}"
...
def notify_super_admin(subject, body, action_type="info"):
    """Envoie une notification au super admin (SMTP - désactivé)"""
    return send_email_notification_smtp(SUPER_ADMIN_EMAIL, subject, body, action_type)

def notify_user_and_admin(user_email, subject, body, action_type="info"):
    """Envoie une notification à l'utilisateur et au super admin (SMTP - désactivé)"""
    # Envoyer à l'utilisateur
    user_sent = send_email_notification_smtp(user_email, subject, body, action_type)
    
    # Envoyer au super admin si différent
    admin_sent = False
    if user_email != SUPER_ADMIN_EMAIL:
        admin_body = f"<p><strong>Notification pour l'utilisateur:</strong> {user_email}</p>" + body
        admin_sent = notify_super_admin(subject, admin_body, action_type)
    
    return user_sent or admin_sent

# ===== FONCTION POUR GÉNÉRER LE LOGO =====
def generate_logo():
    """Charge le logo SETRAF depuis le fichier PNG"""
...
def get_local_ip():
    """Détecte l'adresse IP locale de cette machine"""
    try:
...
# Fonction pour lire les informations du serveur depuis le fichier JSON
@st.cache_data(ttl=60)  # Cache pendant 1 minute
def get_server_info_from_file():
    """Lit le fichier server_info.json créé par le serveur Node.js"""
    try:
        ...
        print(f"Debug - Impossible de lire server_info.json: {e}")
    return None

# Fonction pour détecter l'URL du backend (AVEC CACHE)
@st.cache_data(ttl=600)  # Cache pendant 10 minutes
def get_backend_url():
    """Détermine automatiquement l'URL du backend"""
    import requests
    
    # 0. Vérifier si le fichier server_info.json existe
    server_info = get_server_info_from_file()
    if server_info:
        port = server_info.get('port', 3000)
        local_ip = server_info.get('localIP', 'localhost')
        
        # Essayer l'URL locale d'abord
        try:
            url = f'http://localhost:{port}'
            response = requests.get(url, timeout=1)
            print(f"Debug - Backend trouvé via server_info.json: {url}")
            return url
        except:
            pass
        
        # Essayer l'IP du réseau
        try:
    ...
    # 1. Vérifier si BACKEND_HOST est défini dans .env
    backend_host = os.environ.get('BACKEND_HOST')
    if backend_host and backend_host != 'localhost':
        url = f'http://{backend_host}:3000'
        try:
            response = requests.get(url, timeout=1)
            print(f"Debug - Backend trouvé sur {url}")
            return url
        except:
            pass
    
    # 2. Essayer localhost (même machine)
    try:
        response = requests.get('http://localhost:3000', timeout=1)
        print("Debug - Backend trouvé sur localhost:3000")
    ...
    try:
        ...
        print("Debug - Backend trouvé sur 127.0.0.1:3000")
        return 'http://127.0.0.1:3000'
    except:
        pass
    
    # 4. Essayer l'IP locale de cette machine
    local_ip = get_local_ip()
    if local_ip and local_ip != "127.0.0.1":
        try:
            response = requests.get(f'http://{local_ip}:3000', timeout=1)
            print(f"Debug - Backend trouvé sur {local_ip}:3000")
            return f'http://{local_ip}:3000'
        except:
            pass
    
    # 5. Fallback: retourner localhost (même si non accessible)
    print("Debug - Backend non détecté, utilisation localhost par défaut")
    return 'http://localhost:3000'

# Configuration backend avec détection automatique
BACKEND_URL = get_backend_url()
print(f"Debug - Backend URL finale: {BACKEND_URL}")
...
# Configuration pour le super admin
SUPER_ADMIN_EMAIL = "nyundumathryme@gmail.com"

# Fonction pour obtenir le chemin de la base de données de l'utilisateur
def get_user_db_path(email=None):
    """Retourne le chemin de la base de données spécifique à l'utilisateur"""
    if email is None:
        email = st.session_state.get('user_email', 'default')
    
    # Créer un dossier pour les bases de données utilisateurs
    db_folder = Path("user_databases")
    db_folder.mkdir(exist_ok=True)
    
    # Créer un nom de fichier sécurisé basé sur l'email
    safe_email = email.replace('@', '_at_').replace('.', '_')
    db_path = db_folder / f"user_{safe_email}.db"
    
    return str(db_path)

# Fonction pour vérifier si l'utilisateur est super admin
def is_super_admin():
    """Vérifie si l'utilisateur connecté est le super admin"""
    return st.session_state.get('user_email') == SUPER_ADMIN_EMAIL

# Fonction pour lister toutes les bases de données utilisateurs (super admin only)
def get_all_user_databases():
    """Retourne la liste de toutes les bases de données utilisateurs (super admin uniquement)"""
    ...
    databases = []
    for db_file in db_folder.glob("user_*.db"):
        # Extraire l'email du nom de fichier
        email = db_file.stem.replace('user_', '').replace('_at_', '@').replace('_', '.')
        databases.append({
            'email': email,
            'db_path': str(db_file),
...
# Database path for SQLite (legacy - pour compatibilité)
db_path = "data.db"
...
# Fonction pour vérifier la disponibilité du backend (AVEC CACHE)
@st.cache_data(ttl=60)  # Cache pendant 1 minute
...
# Fonction pour déterminer le mode de fonctionnement (AVEC CACHE)
@st.cache_data(ttl=60)  # Cache pendant 1 minute
def get_app_mode():
...
def init_database(email=None):
    """Initialise la base de données pour un utilisateur spécifique"""
    user_db_path = get_user_db_path(email)
    ...
    # Table pour les métadonnées utilisateur
    cursor.execute('''
    ...
    conn.commit()
    conn.close()
    print(f"Debug - SQLite database initialized for user: {email or 'session user'}")

# Note: L'initialisation sera appelée lors de l'authentification
...
# Fonction pour envoyer des notifications par email (via backend Node.js)
        ...
        if st.button("Vérifier"):
            if otp:
                ...
                if 'error' in result:
                    ...
                    # Initialiser la base de données de l'utilisateur
                    init_database(st.session_state['user_email'])
...
def save_app_data():
    ...
    print(f"Saving data to SQLite for user {user_email}, size: {len(data_json)} chars")
    try:
        user_db_path = get_user_db_path(user_email)
        conn = sqlite3.connect(user_db_path)
        cursor = conn.cursor()
        
        # Supprimer les anciennes données de cet utilisateur
        cursor.execute('DELETE FROM app_data WHERE user_email = ?', (user_email,))
        
        # Insérer les nouvelles données
        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        cursor.execute(
            'INSERT INTO app_data (data, user_email, created_at, updated_at) VALUES (?, ?, ?, ?)',
            (data_json, user_email, timestamp, timestamp)
        )
        
        # Mettre à jour les métadonnées utilisateur
        cursor.execute('''
            INSERT OR REPLACE INTO user_metadata (user_email, created_at, last_access, total_actions)
            VALUES (?, 
                    COALESCE((SELECT created_at FROM user_metadata WHERE user_email = ?), ?),
                    ?,
                    COALESCE((SELECT total_actions FROM user_metadata WHERE user_email = ?), 0) + 1)
        ''', (user_email, user_email, timestamp, timestamp, user_email))
        
        conn.commit()
        conn.close()
        print(f"Données sauvegardées dans SQLite pour {user_email}")
        
        # Envoyer une notification email
        try:
            print(f"📧 Envoi notification enregistrement...")
            total_records = len(data['vehicules']) + len(data['achats']) + len(data['anomalies']) + len(data['habilitations'])
            
            details_text = f"""
            Données sauvegardées avec succès:
            • Véhicules: {len(data['vehicules'])} enregistrement(s)
            • Achats: {len(data['achats'])} enregistrement(s)
            • Anomalies: {len(data['anomalies'])} enregistrement(s)
            • Habilitations: {len(data['habilitations'])} enregistrement(s)
            ...
            send_email_notification(
                action_type="ENREGISTREMENT",
                entity_type="Données",
                entity_id=f"{total_records} enregistrements",
            ...
            if user_email != SUPER_ADMIN_EMAIL:
                send_email_notification(
                    action_type="ENREGISTREMENT UTILISATEUR",
                    entity_type=f"Données de {user_email}",
                    entity_id=f"{total_records} enregistrements",
                    details=details_text,
                    user_info=user_email
                )
            ...
            print(f"⚠️ Erreur notification: {str(email_error)}")
...
def check_expiring_habilitations():
    """Vérifie les habilitations qui expirent bientôt et envoie des rappels"""
    try:
        print("\n🔔 Vérification des habilitations à expirer...")
        
        # Récupérer toutes les bases de données utilisateurs
        all_databases = get_all_user_databases()
        
        habilitations_a_expirer = []
        habilitations_expirees = []
        ...
        for db_info in all_databases:
            try:
                ...
                cursor = conn.cursor()
                cursor.execute('SELECT data FROM app_data WHERE user_email = ? ORDER BY id DESC LIMIT 1', (db_info['email'],))
                result = cursor.fetchone()
                conn.close()
                
                if result:
                    data = json.loads(result[0])
                    habilitations = data.get('habilitations', [])
                    
                    for hab in habilitations:
                        try:
                            date_exp = datetime.strptime(hab['Date_Expiration'], '%Y-%m-%d').date()
                            jours_restants = (date_exp - today).days
                            
                            hab_info = {
                                'user_email': db_info['email'],
                                'employe': hab.get('Employé', 'N/A'),
                                'type': hab.get('Type_Habilitation', 'N/A'),
                                'numero': hab.get('Numéro', 'N/A'),
                                'date_expiration': hab['Date_Expiration'],
                                'jours_restants': jours_restants,
                                'statut': hab.get('Statut', 'N/A')
                            }
                            
                            # Habilitations qui expirent dans les 30 jours
                            if 0 <= jours_restants <= 30:
                                habilitations_a_expirer.append(hab_info)
                            # Habilitations déjà expirées
                            elif jours_restants < 0:
                                habilitations_expirees.append(hab_info)
                        except:
                            continue
            except:
                continue
        ...
        if habilitations_a_expirer or habilitations_expirees:
            print(f"⚠️ {len(habilitations_a_expirer)} habilitation(s) à expirer, {len(habilitations_expirees)} expirée(s)")
            
            # Construire le message
            details_text = "=== HABILITATIONS À RENOUVELER ===\n\n"
            
            if habilitations_expirees:
                details_text += "🔴 EXPIRÉES:\n"
                for hab in habilitations_expirees:
                    details_text += f"  • {hab['employe']} - {hab['type']} ({hab['numero']}) - Expiré depuis {abs(hab['jours_restants'])} jours - Utilisateur: {hab['user_email']}\n"
                details_text += "\n"
            
            if habilitations_a_expirer:
                details_text += "⚠️ À EXPIRER PROCHAINEMENT:\n"
                for hab in habilitations_a_expirer:
                    urgence = "🔴 URGENT" if hab['jours_restants'] <= 7 else "🟡 ATTENTION" if hab['jours_restants'] <= 15 else "🟢 À SURVEILLER"
                    details_text += f"  • {urgence} {hab['employe']} - {hab['type']} ({hab['numero']}) - {hab['jours_restants']} jour(s) - Utilisateur: {hab['user_email']}\n"
            
            # Envoyer au super admin
            send_email_notification(
                action_type="RAPPEL AUTOMATIQUE",
                entity_type="Habilitations",
                entity_id=f"{len(habilitations_a_expirer) + len(habilitations_expirees)} habilitation(s)",
                details=details_text,
                user_info="Système automatique"
            )
            
            # Envoyer à chaque utilisateur concerné
            users_notified = set()
            for hab in habilitations_a_expirer + habilitations_expirees:
                user_email = hab['user_email']
                if user_email not in users_notified:
                    users_notified.add(user_email)
                    
                    # Filtrer les habilitations de cet utilisateur
                    user_habs = [h for h in habilitations_a_expirer + habilitations_expirees if h['user_email'] == user_email]
                    
                    user_details = f"Vos habilitations nécessitent votre attention:\n\n"
                    for h in user_habs:
                        if h['jours_restants'] < 0:
                            user_details += f"🔴 EXPIRÉE: {h['employe']} - {h['type']} - Expiré depuis {abs(h['jours_restants'])} jours\n"
                        else:
                            user_details += f"⚠️ EXPIRE BIENTÔT: {h['employe']} - {h['type']} - {h['jours_restants']} jour(s) restant(s)\n"
                    
                    send_email_notification(
                        action_type="RAPPEL",
                        entity_type="Vos habilitations",
                        entity_id=f"{len(user_habs)} habilitation(s)",
                        details=user_details,
                        user_info=user_email
                    )
            
            print(f"✅ Rappels envoyés à {len(users_notified)} utilisateur(s) + super admin")
            return True
        else:
            print("✅ Aucune habilitation à expirer dans les 30 jours")
            return False
            
    except Exception as e:
        print(f"❌ Erreur vérification habilitations: {str(e)}")
        import traceback
        traceback.print_exc()
        return False
...
def load_app_data():
    ...
    try:
        ...
        cursor.execute('SELECT data FROM app_data WHERE user_email = ? ORDER BY id DESC LIMIT 1', (user_email,))
        result = cursor.fetchone()
        conn.close()
        ...
        if result:
            ...
            print(f"Données chargées depuis SQLite pour {user_email}")
            ...
            print(f"Aucune donnée trouvée dans SQLite pour {user_email}")
            return False
    except Exception as e:
...
def sync_data_after_save():
    """Synchronise les données en mémoire avec celles de la base de données SQLite"""
    try:
        print("🔄 Synchronisation des données après sauvegarde...")

        # Charger les données depuis SQLite uniquement
        try:
            user_email = st.session_state.get('user_email', 'default')
            user_db_path = get_user_db_path(user_email)
            conn = sqlite3.connect(user_db_path)
        ...
        if sqlite_data:
            for key in ['vehicules', 'achats', 'anomalies', 'habilitations']:
                sqlite_items = sqlite_data.get(key, [])
            ...
            print("✅ Données synchronisées depuis SQLite")
            return True
        else:
...
def export_to_excel():
    """Exporte toutes les données dans un fichier Excel avec plusieurs onglets"""
    try:
        ...
        # Onglet Véhicules
        if 'vehicules' in st.session_state and not st.session_state.vehicules.empty:
            ws_vehicules = wb.active
            if ws_vehicules:
                ws_vehicules.title = "Véhicules"
            
                # Écrire les en-têtes
                headers = list(st.session_state.vehicules.columns)
                for col_num, header in enumerate(headers, 1):
                    cell = ws_vehicules.cell(row=1, column=col_num, value=header)
                    cell.font = header_font
                    cell.fill = header_fill
                
                # Écrire les données
                for row_num, row in enumerate(st.session_state.vehicules.itertuples(index=False), 2):
                    for col_num, value in enumerate(row, 1):
                        ws_vehicules.cell(row=row_num, column=col_num, value=value)
        
        # Onglet Achats
        if 'achats' in st.session_state and not st.session_state.achats.empty:
            ws_achats = wb.create_sheet("Achats")
            
            headers = list(st.session_state.achats.columns)
            for col_num, header in enumerate(headers, 1):
                cell = ws_achats.cell(row=1, column=col_num, value=header)
                cell.font = header_font
                cell.fill = header_fill
            
            for row_num, row in enumerate(st.session_state.achats.itertuples(index=False), 2):
                for col_num, value in enumerate(row, 1):
                    ws_achats.cell(row=row_num, column=col_num, value=value)
        
        # Onglet Anomalies
        if 'anomalies' in st.session_state and not st.session_state.anomalies.empty:
            ws_anomalies = wb.create_sheet("Anomalies")
            
            headers = list(st.session_state.anomalies.columns)
            for col_num, header in enumerate(headers, 1):
                cell = ws_anomalies.cell(row=1, column=col_num, value=header)
                cell.font = header_font
                cell.fill = header_fill
            
            for row_num, row in enumerate(st.session_state.anomalies.itertuples(index=False), 2):
                for col_num, value in enumerate(row, 1):
                    ws_anomalies.cell(row=row_num, column=col_num, value=value)
        
        # Onglet Habilitations
        if 'habilitations' in st.session_state and not st.session_state.habilitations.empty:
            ws_habilitations = wb.create_sheet("Habilitations")
            
            headers = list(st.session_state.habilitations.columns)
            for col_num, header in enumerate(headers, 1):
                cell = ws_habilitations.cell(row=1, column=col_num, value=header)
                cell.font = header_font
                cell.fill = header_fill
            
            for row_num, row in enumerate(st.session_state.habilitations.itertuples(index=False), 2):
                for col_num, value in enumerate(row, 1):
                    ws_habilitations.cell(row=row_num, column=col_num, value=value)
        
        # Sauvegarder le fichier
        wb.save(filepath)
        
        # Log de l'export
        log_action("EXPORT", "base_données", "Excel", f"Fichier exporté: {filename}")
...
# Fonction pour sauvegarder les données dans SQLite
def init_audit_logs(email=None):
    # Use SQLite
    user_db_path = get_user_db_path(email)
    conn = sqlite3.connect(user_db_path)
    cursor = conn.cursor()
...
def log_action_sqlite(action_type, entity_type, entity_id="", details="", user_info="Utilisateur"):
    try:
        ...
        cursor.execute('''
            INSERT INTO audit_logs (timestamp, user_action, entity_type, entity_id, action_type, details, user_info, user_email)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (timestamp, f"{action_type} {entity_type}", entity_type, entity_id, action_type, details, user_info, user_email))
...
def add_to_corbeille(entity_type, entity_data, deleted_by="Utilisateur"):
    ...
    try:
        ...
        cursor.execute('''
            INSERT INTO corbeille (entity_type, entity_data, deleted_at, deleted_by, user_email)
            VALUES (?, ?, ?, ?, ?)
        ''', (entity_type, json.dumps(entity_data), datetime.now().strftime('%Y-%m-%d %H:%M:%S'), deleted_by, user_email))
...
def restore_from_corbeille(item_id, user_info="Utilisateur"):
    ...
    try:
        ...
        cursor.execute('SELECT entity_type, entity_data FROM corbeille WHERE id = ? AND user_email = ?', (item_id, user_email))
...
def empty_corbeille(user_info="Utilisateur"):
    ...
    try:
        ...
        cursor.execute('SELECT COUNT(*) FROM corbeille WHERE user_email = ?', (user_email,))
        result = cursor.fetchone()
        count = result[0] if result else 0
        
        # Vider la corbeille
        cursor.execute('DELETE FROM corbeille WHERE user_email = ?', (user_email,))
...
def get_database_stats():
    ...
    try:
        ...
        db_size_bytes = os.path.getsize(user_db_path) if os.path.exists(user_db_path) else 0
        ...
        # Statistiques des logs d'audit (SQLite uniquement)
        try:
            conn = sqlite3.connect(user_db_path)
            cursor = conn.cursor()
            cursor.execute('SELECT COUNT(*) FROM audit_logs WHERE user_email = ?', (user_email,))
            result = cursor.fetchone()
        ...
        with col2:
            if st.button("🗑️ Vider les logs", width='stretch', type="primary"):
                if st.session_state.get('confirm_clear_logs', False):
                    ...
                    conn = sqlite3.connect(user_db_path)
                    cursor = conn.cursor()
                    cursor.execute('DELETE FROM audit_logs WHERE user_email = ?', (user_email,))
                    conn.commit()
                    conn.close()
...
def suivi_achats():
    ...
    with tab3:
        if not st.session_state.achats.empty:
            ...
            # Convertir Period en string pour Plotly
            achats_tendance['Mois'] = achats_tendance['Mois'].astype(str)
            
            # Optimisation: limiter le nombre de points si trop de données
            if len(achats_tendance) > 1000:
                achats_tendance = achats_tendance.sample(n=1000)
            
            # Utiliser un line chart au lieu de scatter pour les tendances
            fig = px.line(achats_tendance, x='Mois', y='Prix_EUR', 
                           color='Catégorie',
                           labels={'Mois': 'Mois', 'Prix_EUR': 'Prix (EUR)', 'Catégorie': 'Catégorie'},
                           title="Tendances d'achat: Évolution des prix par mois et catégorie")
            fig.update_layout(height=400)  # Hauteur fixe pour performances
            st.plotly_chart(fig, width='stretch', key="tendance_achats")
            ...
            if st.button("💾 Sauvegarder", width='stretch'):
                if employe and type_habilitation and organisme and date_expiration:
                    ...
                    if st.session_state.edit_mode == 'edit_habilitation':
                        ...
                        # Notification email pour modification d'habilitation
                        try:
                            user_email = st.session_state.get('user_email', 'default')
                            details_text = f"""
                            Employé: {hab_dict['Employé']}
                            Type: {hab_dict['Type_Habilitation']}
                            Numéro: {hab_dict['Numéro']}
                            Date d'obtention: {hab_dict['Date_Obtention']}
                            Date d'expiration: {hab_dict['Date_Expiration']}
                            Organisme: {hab_dict['Organisme']}
                            Statut: {hab_dict['Statut']}
                            Vérifié par: {hab_dict['Vérifié_Par']}
                            """
                            send_email_notification(
                                action_type="MODIFICATION",
                                entity_type="Habilitation",
                                entity_id=hab_dict['Employé'],
                                details=details_text,
                                user_info=user_email
                            )
                            print(f"✅ Notification habilitation envoyée!")
                        except Exception as e:
                        ...
                        # Notification email pour nouvelle habilitation
                        try:
                            user_email = st.session_state.get('user_email', 'default')
                            print(f"📧 Envoi notification habilitation...")
                            jours_restants = (datetime.strptime(hab_dict['Date_Expiration'], '%Y-%m-%d').date() - datetime.now().date()).days
                            ...
                            Date d'obtention: {hab_dict['Date_Obtention']}
                            Date d'expiration: {hab_dict['Date_Expiration']}
                            Jours restants: {jours_restants} jours
                            Organisme: {hab_dict['Organisme']}
                            Statut: {hab_dict['Statut']}
                            Vérifié par: {hab_dict['Vérifié_Par']}
                            Date de vérification: {hab_dict['Date_Vérification']}
                            """
                            ...
                            print(f"✅ Notification habilitation envoyée!")
                            st.info("📧 Email de notification envoyé")
                        except Exception as e:
                            print(f"⚠️ Erreur notification: {str(e)}")
...
# Page Super Admin - Gestion de tous les utilisateurs
def super_admin_page():
    """Page réservée au super administrateur pour gérer tous les utilisateurs"""
    if not is_super_admin():
        st.error("⛔ Accès refusé. Cette page est réservée au super administrateur.")
        return
    
    st.markdown('<div class="main-header">', unsafe_allow_html=True)
    st.title("🔐 Panneau Super Administrateur")
    st.markdown("Gestion centralisée de tous les utilisateurs et leurs données")
    st.markdown('</div>', unsafe_allow_html=True)
    
    st.markdown("---")
    
    # Bouton de vérification manuelle des habilitations
    col_btn1, col_btn2 = st.columns(2)
    with col_btn1:
        if st.button("🔔 Vérifier les habilitations à expirer", width='stretch'):
            with st.spinner('Vérification en cours...'):
                if check_expiring_habilitations():
                    st.success("✅ Rappels envoyés! Vérifiez vos emails.")
                else:
                    st.info("ℹ️ Aucune habilitation à expirer dans les 30 jours.")
    ...
    if not all_databases:
        st.warning("⚠️ Aucune base de données utilisateur trouvée.")
        return
    ...
    total_users = len(all_databases)
    total_size = sum([db['size'] for db in all_databases])
    
    with col1:
        st.metric("👥 Utilisateurs totaux", total_users)
    with col2:
        st.metric("💾 Espace total utilisé", f"{total_size / (1024*1024):.2f} MB")
    with col3:
        st.metric("📊 Espace moyen/utilisateur", f"{total_size / (1024*1024*total_users):.2f} MB" if total_users > 0 else "0 MB")
    with col4:
        # Calculer le total d'enregistrements dans toutes les bases
        total_records = 0
        for db_info in all_databases:
            try:
                conn = sqlite3.connect(db_info['db_path'])
                ...
                pass
        st.metric("📝 Enregistrements totaux", total_records)
    ...
    # Créer un DataFrame avec les informations des utilisateurs
    users_data = []
    for db_info in all_databases:
        try:
            conn = sqlite3.connect(db_info['db_path'])
            ...
            cursor.execute("SELECT COUNT(*) FROM app_data WHERE user_email = ?", (db_info['email'],))
            result = cursor.fetchone()
            app_data_count = result[0] if result else 0
            
            cursor.execute("SELECT COUNT(*) FROM audit_logs WHERE user_email = ?", (db_info['email'],))
            result = cursor.fetchone()
            audit_count = result[0] if result else 0
            
            cursor.execute("SELECT last_access FROM user_metadata WHERE user_email = ?", (db_info['email'],))
            last_access = cursor.fetchone()
            last_access_str = last_access[0] if last_access and last_access[0] else "N/A"
            
            conn.close()
            
            users_data.append({
                'Email': db_info['email'],
                'Taille DB (MB)': f"{db_info['size'] / (1024*1024):.2f}",
                'Dernière modification': db_info['modified'].strftime('%Y-%m-%d %H:%M'),
                'Dernier accès': last_access_str,
                'Données': app_data_count,
                'Logs': audit_count
            })
        except Exception as e:
            users_data.append({
    ...
    # Section de consultation des données d'un utilisateur
    st.markdown("### 🔍 Consulter les Données d'un Utilisateur")
    
    selected_user = st.selectbox(
        "Sélectionner un utilisateur :",
        [db['email'] for db in all_databases]
    )
    
    if selected_user:
        user_db = next((db for db in all_databases if db['email'] == selected_user), None)
        
        if user_db:
            st.info(f"📂 Base de données : `{user_db['db_path']}`")
            
            col1, col2 = st.columns(2)
            
            with col1:
                if st.button("📊 Voir les données principales", key="view_main_data"):
                    try:
                        conn = sqlite3.connect(user_db['db_path'])
                        cursor = conn.cursor()
                        cursor.execute('SELECT data FROM app_data WHERE user_email = ? ORDER BY id DESC LIMIT 1', (selected_user,))
                        result = cursor.fetchone()
                        conn.close()
                        ...
                        if result:
                            ...
                            for key in ['vehicules', 'achats', 'anomalies', 'habilitations']:
                                with st.expander(f"📂 {key.capitalize()} ({len(data.get(key, []))} entrées)"):
                                    if data.get(key):
                                        df = pd.DataFrame(data[key])
                                        st.dataframe(df, width='stretch')
                                    else:
                                        st.info(f"Aucune donnée pour {key}")
                        else:
            ...
            with col2:
                if st.button("📝 Voir les logs d'audit", key="view_audit_logs"):
                    try:
                        conn = sqlite3.connect(user_db['db_path'])
                        cursor = conn.cursor()
                        cursor.execute('''
                            SELECT timestamp, user_action, entity_type, entity_id, action_type, details
                            FROM audit_logs
                            WHERE user_email = ?
                            ORDER BY timestamp DESC
                            LIMIT 50
                        ...
                        if logs:
                            df_logs = pd.DataFrame(logs, columns=['Timestamp', 'Action', 'Type', 'ID', 'Action Type', 'Détails'])
                            st.dataframe(df_logs, width='stretch')
                        else:
                            st.info("Aucun log trouvé pour cet utilisateur")
                    except Exception as e:
                        st.error(f"Erreur lors de la récupération des logs : {str(e)}")
    
    st.markdown("---")
    
    # Visualisation graphique de l'utilisation
    st.markdown("### 📈 Analyse de l'Utilisation")
    
    # Graphique de l'espace utilisé par utilisateur
    fig = px.bar(df_users, x='Email', y='Taille DB (MB)',
                 title="Espace disque utilisé par utilisateur",
                 labels={'Taille DB (MB)': 'Taille (MB)', 'Email': 'Utilisateur'},
                 color='Taille DB (MB)',
                 color_continuous_scale='Blues')
    fig.update_layout(xaxis_tickangle=-45)
    st.plotly_chart(fig, width='stretch')
    
    # Graphique du nombre de données par utilisateur
    fig2 = px.bar(df_users, x='Email', y='Données',
                  title="Nombre d'enregistrements par utilisateur",
                  labels={'Données': 'Enregistrements', 'Email': 'Utilisateur'},
                  color='Données',
                  color_continuous_scale='Greens')
    ...
    with st.sidebar:
        ...
        if is_super_admin():
            page = st.radio(
                ...
                ["📊 Tableau de Bord", "🚙 Inspection Véhicules", 
                 ...
                 "🎓 Habilitations", "🗑️ Corbeille", "🔐 Super Admin"],
        ...
        if st.button("💾 Sauvegarder dans SQLite", width='stretch'):
            with st.spinner('💾 Sauvegarde en cours...'):
                if save_app_data():
                    st.success("✅ Données sauvegardées!")
                    st.info("📧 Emails de notification envoyés à l'utilisateur et au super admin")
                else:
                    st.error("❌ Erreur lors de la sauvegarde")
        ...
        # Export vers Excel
        if st.button("📊 Exporter vers Excel", width='stretch'):
            with st.spinner("Export en cours..."):
                excel_file = export_to_excel()
                if excel_file:
                    st.success(f"✅ Export réussi! Fichier: {os.path.basename(excel_file)}")
...
def gestion_corbeille():
    ...
    with col2:
        if st.button("🗑️ Vider la corbeille", type="secondary", width='stretch'):
    ...
    if 'empty_corbeille_confirm' in st.session_state and st.session_state['empty_corbeille_confirm']:
        with st.container():
            ...
            col1, col2, col3 = st.columns([1, 1, 1])
            with col1:
                if st.button("📧 Envoyer OTP", width='stretch'):
        ...
        if vehicules_corbeille:
            for item in vehicules_corbeille:
                ...
                with st.container():
                    ...
                    with col2:
                        if st.button("🔄 Restaurer", key=f"restore_veh_{item['id']}", width='stretch'):
            ...
            for item in achats_corbeille:
                ...
                with st.container():
                    ...
                    with col1:
                        ...
                        st.caption(f"Supprimé le: {item['deleted_at']} par {item['deleted_by']}")
                    with col2:
                        if st.button("🔄 Restaurer", key=f"restore_achat_{item['id']}", width='stretch'):
                            success, message = restore_from_corbeille(item['id'])
                            if success:
                                st.success(f"✅ {message}")
                                st.rerun()
        ...
        if anomalies_corbeille:
            for item in anomalies_corbeille:
                ...
                with st.container():
                    ...
                    with col2:
                        if st.button("🔄 Restaurer", key=f"restore_anomalie_{item['id']}", width='stretch'):
                            success, message = restore_from_corbeille(item['id'])
                            if success:
                                st.success(f"✅ {message}")
                                st.rerun()
        ...
        if habilitations_corbeille:
            for item in habilitations_corbeille:
                ...
                with st.container():
                    ...
                    with col2:
                        if st.button("🔄 Restaurer", key=f"restore_hab_{item['id']}", width='stretch'):
                            success, message = restore_from_corbeille(item['id'])
                            if success:
                                st.success(f"✅ {message}")
                                st.rerun()
...
def get_database_info():
    ...
    try:
        ...
        # Taille du fichier de base de données
        db_size = os.path.getsize(user_db_path) if os.path.exists(user_db_path) else 0
        ...
        for table in tables:
            try:
                cursor.execute(f'SELECT COUNT(*) FROM {table} WHERE user_email = ?', (user_email,))
                result = cursor.fetchone()
                count = result[0] if result else 0
        ...
        cursor.execute('SELECT COUNT(*), MAX(timestamp) FROM audit_logs WHERE user_email = ?', (user_email,))
...
def main():
    ...
    if 'app_started_logged' not in st.session_state:
        ...
        # Vérification automatique des habilitations à expirer (une fois par session)
        if 'habilitations_checked' not in st.session_state:
            try:
                check_expiring_habilitations()
                st.session_state.habilitations_checked = True
            except Exception as e:
                print(f"⚠️ Erreur vérification habilitations au démarrage: {str(e)}")
