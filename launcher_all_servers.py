...
def launch_mariadb():
    ...
    bat_path = Path(__file__).parent / "start_mariadb.bat"
    ...
    try:
        subprocess.Popen([str(bat_path)], cwd=os.getcwd())
...
def launch_node_server():
    ...
    server_path = Path(__file__).parent / "excel" / "server.js"
    ...
    try:
        subprocess.Popen(["node", str(server_path)], cwd=server_path.parent)
...
def launch_excel_app():
    ...
    app_path = Path(__file__).parent / "excel" / "app.py"
    ...
    try:
        subprocess.Popen(["streamlit", "run", str(app_path)], cwd=app_path.parent)
